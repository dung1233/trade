...
const handleRestaurantSelect = (restaurant) => {
  // Lưu thông tin nhà hàng vào localStorage dưới khóa 'addressSelect'
  localStorage.setItem('addressSelect', JSON.stringify({
...
useEffect(() => {
  ...
  const savedMenu = localStorage.getItem('menuSelect');
  const savedReservation = localStorage.getItem('reservation');
  const savedAddressSelect = localStorage.getItem('addressSelect');

  // Nếu có dữ liệu về menu đã chọn, cập nhật state
  if (savedMenu) {
      setSelectedMenu(JSON.parse(savedMenu));
  }

  // Nếu có dữ liệu về thông tin đặt chỗ, cập nhật state
  if (savedReservation) {
      const reservation = JSON.parse(savedReservation);
      setReservationDetails(reservation);
  }

  // Nếu có dữ liệu về nhà hàng, cập nhật state và thêm thông tin vào reservationDetails
  if (savedAddressSelect) {
      const addressData = JSON.parse(savedAddressSelect);
      setReservationDetails(prevDetails => ({
          ...prevDetails,
          restaurantId: addressData.restaurantId,
          restaurantName: addressData.restaurantName,
          address: addressData.address,
          description: addressData.description,
          rating: addressData.rating
      }));
  }

  // Log để kiểm tra dữ liệu đã load
...
const calculateSubTotal = () => {
    ...
    Object.values(selectedMenu).forEach(item => {
        if (item.categoryId === 4) {
            subTotal += item.price * person;
        } else if ([1, 2, 3].includes(item.category)) {
...
const handleCheckout = async () => {
  try {
      // Lấy dữ liệu về thời gian sự kiện
      ...
      const orderPayload = {
          ...
          restaurant_id: reservationDetails.restaurantId || 0,
      ...
      // Gửi chi tiết các món trong đơn hàng
      for (const item of Object.values(selectedMenu)) {
          const detailPayload = {
              orderId: orderId,
...
// Function to handle popup and reload data from localStorage
function openConfirmPopup() {
  const savedMenu = JSON.parse(localStorage.getItem("menuSelect")) || {};
  const savedReservation = JSON.parse(localStorage.getItem("reservation")) || {};

  setSelectedMenu(savedMenu);
  setReservationDetails(savedReservation);
            ...
            <div className="banner-text">
              <div className="banner-cell">
                ...
                <div className="book-btn">
                  <a href="#reservation" className="table-btn hvr-underline-from-center" onClick={openPopupBooking}>
                    Book my Table
                  </a>
                          ...
                          <div className="menu-confirmation">
                            ...
                            <div className="content">
                                  ...
                                  {[1, 2, 3, 4].map(category => (
                                    <div key={category} style={{ marginBottom: '20px' }}>
                                      ...
                                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                        <tbody>
                                          {Object.values(selectedMenu).filter(item => item.category === category).map((item, index) => {
                                            let quantity = category === 4 ? reservationDetails.person : Math.ceil(reservationDetails.person / reservationDetails.selectPerson);
                                            return (
                                              <tr key={index}>
                                                <td style={{ padding: '8px', width: "60%" }}>{item.itemName}</td>
                              ...
                              <section className="delivery-info">
                                ...
                                <p><strong>Địa điểm tổ chức:</strong> {reservationDetails.restaurantName || "Loading..."}</p>
