...
const isValidDate = (date) => {
  ...
  const filterMenu = (categoryId) => {
    if (categoryId === null) {
      setFilteredItems(menuItems); // Hiển thị tất cả món ăn nếu chọn "All"
    } else {
  ...
  useEffect(() => {
    if (searchQuery === "") {
      setFilteredItems(menuItems); // Cập nhật danh sách nếu không có query tìm kiếm
    }
  }, [menuItems, searchQuery, selectedSizes]);  // Theo dõi sự thay đổi trong menuItems và selectedSizes
  ...
  useEffect(() => {
    ...
    Object.keys(selectedSizes).forEach((key) => {
      const item = selectedSizes[key];
      const categoryId = item.category;
  
      if (updatedCategorizedItems[categoryId]) {
        updatedCategorizedItems[categoryId].items.push(item);
      }
    });
  
    setCategorizedItems(updatedCategorizedItems);
  }, [selectedSizes]);  // thêm selectedSizes để cập nhật khi selectedSizes thay đổi
...
// };
const removeFromOrder = (categoryId, index) => {
  // Lấy dữ liệu menuSelect từ localStorage
  const menuSelect = JSON.parse(localStorage.getItem('menuSelect'));

  // Kiểm tra xem menuSelect và categoryId có tồn tại và items không phải undefined
  if (menuSelect && menuSelect[categoryId] && Array.isArray(menuSelect[categoryId].items)) {
    // Kiểm tra xem index có hợp lệ không (nếu index vượt quá phạm vi mảng, không làm gì cả)
    if (index >= 0 && index < menuSelect[categoryId].items.length) {
      // Xóa phần tử tại vị trí index trong items của categoryId
      menuSelect[categoryId].items.splice(index, 1);

      // Cập nhật lại dữ liệu vào localStorage
      localStorage.setItem('menuSelect', JSON.stringify(menuSelect));
      
      // Cập nhật lại state nếu cần (ví dụ: để giao diện tự động cập nhật)
      // setState(menuSelect); // Uncomment nếu sử dụng state trong React
    }
  } else {
    console.error(`Không tìm thấy categoryId ${categoryId} hoặc items không phải là mảng.`);
  }
};
  // useEffect(() => {
  //   generateDates();
      ...
      <div id="popup-booking" className="popup row" style={{ width: '80%' }}>
        ...
        <div className="order col-lg-4 col-sm-5 col-xs-12">
          ...
          {[1, 2, 3, 4].map(categoryId => (
            categorizedItems[categoryId]?.items?.length > 0 && (
              <div key={categoryId} style={{ marginBottom: '20px' }}>
                <h3>{categorizedItems[categoryId].name}</h3>
                {categorizedItems[categoryId].items.map((item, index) => (
                  <div key={index} style={{ display: "flex", justifyContent: "space-between", marginBottom: "7px" }}>
                    <h3 style={{ width: "70%", paddingTop: "2px", paddingBottom: "0" }}>
                      {`${item.itemName} (Size: ${item.sizeId})`}
                    </h3>
                    <span style={{ width: "20%", fontSize: "16px" }}>{`$${item.price.toFixed(2)}`}</span>
                    <button style={{ width: "10%", padding: "0 5px 5px 5px" }} onClick={() => removeFromOrder(categoryId, index)}>
                      X
                    </button>
                  </div>
                ))}
