...
export default function Banner({ onClose }) {
  ...
  const toggleDish = (dish) => {
    if (!dish) {
      console.error('Dish is not defined or was not passed correctly.');
      return;
    }

    setReservationDetails((prev) => {
      const selectedDishes = [...prev.selectedDishes];
      const index = selectedDishes.findIndex((item) => item.id === dish.menuItemNo);

      if (index >= 0) {
        selectedDishes.splice(index, 1); // Bỏ món
      } else {
        if (dish.itemVariants && dish.itemVariants.length > 0) {
          const defaultVariant = dish.itemVariants[0]; // Variant đầu tiên
          selectedDishes.push({
            id: dish.menuItemNo,
            name: dish.itemName,
            selectedVariant: {
          ...
          console.warn(`Item ${dish.itemName} does not have variants.`);
        }
      }
  ...
  // Hàm cập nhật variant (size) và giá khi người dùng đổi size
  const updateDishVariant = (dishId, newVariant) => {
    setReservationDetails((prev) => {
      const selectedDishes = [...prev.selectedDishes];
      const index = selectedDishes.findIndex((item) => item.id === dishId);

      if (index >= 0) {
        selectedDishes[index].selectedVariant = {
  ...
  // Hàm xử lý thay đổi sizeId
const handleSizeChange = (menuItemNo, direction) => {
  setReservationDetails((prevDetails) => {
    return prevDetails.map((item) => {
      if (item.menuItemNo === menuItemNo) {
        let newSizeId = item.sizeId + (direction === "increase" ? 1 : -1);

        // Đảm bảo sizeId không vượt quá phạm vi
        const sizeIds = item.variants.map((variant) => variant.sizeId);
        if (newSizeId < Math.min(...sizeIds) || newSizeId > Math.max(...sizeIds)) {
          return item; // Không thay đổi nếu vượt phạm vi
        }

        // Tìm variant tương ứng với newSizeId
        const newVariant = item.variants.find(
          (variant) => variant.sizeId === newSizeId
        );

        return {
          ...item,
          sizeId: newSizeId,
          price: newVariant.price, // Cập nhật giá theo variant mới
        };
      }
      return item;
    });
...
// Hàm xóa món
const handleRemoveItem = (menuItemNo) => {
  setReservationDetails((prevDetails) =>
    prevDetails.filter((item) => item.menuItemNo !== menuItemNo)
  );
};
  return (
                          ...
                          <>
                            ...
                            <div style={{ display: 'flex', height: '76vh', fontFamily: 'Arial, sans-serif', border: '1px solid #efd8d8' }}>
                              ...
                              >
                                ...
                                <div style={{ flexGrow: 1, border: '1px dashed #ddd', padding: '10px', marginBottom: '20px', color: 'black' }}>
                                    ...
                                    reservationDetails.selectedDishes.map((dish, index) => (
                                      <div key={index} style={{ marginBottom: '20px' }}>
                                        ...
                                        <div style={{ marginTop: '10px' }}>
                                          {/* Danh sách size */}
                                          {menuItems
                                            .find((item) => item.menuItemNo === dish.id)
                                            ?.itemVariants.map((variant) => (
                                              <button
                                                key={variant.variantId}
                                                onClick={() => updateDishVariant(dish.id, variant)} // Đổi size
                                                style={{
                                                  padding: '5px 10px',
                                                  margin: '5px',
                                                  backgroundColor:
                                                    dish.selectedVariant.variantId === variant.variantId ? '#4CAF50' : '#ddd',
                                                  color: dish.selectedVariant.variantId === variant.variantId ? 'white' : 'black',
                                                  border: 'none',
                                                  borderRadius: '5px',
                                                  cursor: 'pointer',
                                                  fontSize:'10px',
                                              ...
                                              >
                                                Size {variant.sizeId} - ${variant.price.toFixed(2)}
                                              </button>
                                            ))}
                                        </div>
                                      </div>
                                ...
                                <div style={{ fontWeight: 'bold', marginBottom: '20px', color: 'black' }}>
                                  ...
                                  {reservationDetails.selectedDishes
                                    .reduce((total, dish) => total + dish.selectedVariant.price, 0)
                                    .toFixed(2)}
                                </div>
                                <div style={{ fontWeight: 'bold', marginBottom: '20px', color: 'black' }}>Total: ${calculateTotal()}</div>
                                <button
